...
float4 PSnightvision( float2 Tex : TEXCOORD0  ) : COLOR0 
{
	...
	*/
	...
	float2 sc; sincos(Tex.y * 768-(time*100), sc.x, sc.y);//scrolling scanlines (remember faster in FPSC!)
	float2 bc; sincos(Tex.y -(time*100), bc.x, bc.y); //blinking screen (remember faster in FPSC!)
...
>
{
	...
	>
	{
		...
		PixelShader = compile ps_2_0 PSdreamy();
	}
}
