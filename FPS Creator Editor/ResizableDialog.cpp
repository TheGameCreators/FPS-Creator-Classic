...
// [[ begin added by Mike Melnikov
void CResizableDialog::ReAddAnchor(HWND wnd)
{
	int i, count = m_arrLayout.GetSize();
	for (i=0; i<count; ++i)
	{
    if(m_arrLayout[i].hwnd==wnd)
    {
      AddAnchor(wnd,m_arrLayout[i].tl_type,m_arrLayout[i].br_type);
  		m_arrLayout.RemoveAt(i);
      break;
    }
  }
}
// end added by Mike Melnikov ]]
